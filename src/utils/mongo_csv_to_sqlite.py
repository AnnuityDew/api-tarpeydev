...
async def init_from_mongo():
    ...
    with Session(engine) as session, open(
        ...
        # select it all back out for viewing
        sql = select(SimulatedBracketORM)
        result = session.execute(sql).scalars().all()
        print(result)
